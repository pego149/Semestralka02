...
namespace structures
{
	...
	{
		return dynamic_cast<BinaryTreeNode<T>*>(children_->clone());
	}
	...
	{
		return getParent() != nullptr && getParent()->getLeftSon() == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::isRightSon() const
	{
		return getParent() != nullptr && getParent()->getRightSon() == this;
	}
	...
	{
		BinaryTreeNode<T>* currentBinaryNode = dynamic_cast<BinaryTreeNode<T>*>(current);

		if (currentBinaryNode != nullptr) {
			populatePath(currentBinaryNode->getLeftSon());
			path_->push(currentBinaryNode);
			populatePath(currentBinaryNode->getRightSon());
		}
	}
