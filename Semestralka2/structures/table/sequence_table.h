...
namespace structures 
{
	...
	public:
		...
		void insertValid(const K& key, const T& data) override;
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		{
			throw std::invalid_argument("No such key arg");
		}
	}
	...
	template<typename K, typename T>
	inline void SequenceTable<K, T>::insertValid(const K & key, const T & data)
	{
		list_->add(new TableItem<K, T>(key, data));
	}
	...
	inline T SequenceTable<K, T>::remove(const K & key)
	{
		...
		{
			throw std::invalid_argument("No such key in table");
		}
	}
